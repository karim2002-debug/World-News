...
final class APICaller{
    struct Constant{
        static let API_Key = "pub_115299bf6140713371ee13ed9d4bd1b84099b"
        static var articalTitle : [String]!
    }
    static func getNews(completationHandler :@escaping( Result<[Results],Error>)->Void){
        
        guard let url = URL(string: "https://newsdata.io/api/1/latest?apikey=pub_11529a6414f54929aafd7a4de17988c11201d&q=pegasus&language=en")else{
            return
        }
    ...
    static func getsearchNews(completationHandler :@escaping( Result<[Results],Error>)->Void){
        ...
        guard let url = URL(string: "https://newsdata.io/api/1/news?apikey=pub_11529a6414f54929aafd7a4de17988c11201d")else{
            return
        }
        AF.request(url,method: .get).responseDecodable(of: NewsModel.self) { response in
            switch response.result{
    ...
    static func searchForNews(query : String , completionHandler : @escaping(Result<[Results],Error>)->Void){
        ...
        guard let url = URL(string: "https://newsdata.io/api/1/news?apikey=pub_11529a6414f54929aafd7a4de17988c11201d&q=\(query)")else{
            return
        }
